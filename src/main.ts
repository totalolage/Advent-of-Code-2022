...
export const main = (input: string, opts: {mode: 'sum', maxSize: number} | {mode: 'print'} | {mode: 'delete', spaceNeeded: number, totalSpace: number}) => {
    interface InodeBase {
        name: string,
        type: 'dir' | 'file',
    }
    ...
    switch (opts.mode) {
        ...
        case 'delete': {
            const allDirectoriesWithSizes: (Directory & {size: number})[] = [];
            const calculateSize = (dir: Directory): number => {
                let size = 0;
                for (const item of dir.content) {
                    if (isDirectory(item)) {
                ...
                allDirectoriesWithSizes.push({ ...dir, size });
                return size;
            }
            calculateSize(currentDir);

            const rootDir = allDirectoriesWithSizes.find(dir => dir.parent === null);
            if (rootDir === undefined) {
                throw new Error(`Root directory not found`);
            }

            const spaceAvailable = opts.totalSpace - rootDir.size;
            if (spaceAvailable < 0) {
                throw new Error(`Not enough space`);
            }

            const spaceNeededToBeFreed = opts.spaceNeeded - spaceAvailable;
            if (spaceNeededToBeFreed < 0) {
                return 0;
            }

            // Find the smallest directory that would free up enough space
            const smallestSufficientDirectory = allDirectoriesWithSizes
                .reduce((smallest, dir) => {
                    if (dir.size >= spaceNeededToBeFreed && (!smallest || dir.size < smallest.size)) {
                        return dir;
                    }
                    return smallest;
                },
                undefined as (Directory & {size: number}) | undefined
            );

            if (smallestSufficientDirectory === undefined) {
                throw new Error(`Not enough space`);
            }

            return smallestSufficientDirectory.size;
        }
    }
}